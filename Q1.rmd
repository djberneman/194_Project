...
for(i in 0:100){
  ...
  for(j in 1:97){
    ...
    Large_Cap = alpha * data2$Large_Cap[j]
    TR = long_corp+intermediate_gov+Large_Cap
    Conf2a[j, i+2] = TR}}
Conf2a$Year = data2$Year
...
for(i in 0:100){
  ...
  for(j in 1:97){
    ...
    TR = long_corp+intermediate_gov+Large_Cap
    Conf2b[j, i+2] = TR}}
Conf2b$Year = data2$Year

vect1 = vector()
for(i in 2:102){
  vect1 = append(vect1, mean(Conf2a[,i]))}

vect2 = vector()
for(i in 2:102){
  vect2 = append(vect2, mean(Conf2b[,i]))}
plot(x=0:100, y = vect2)

data3 = data.frame(matrix(nrow=69, ncol=102))
colnames(data3) = c("Year", colnames(data)[4:104])
data3$Year[1:68] = data$Date[1:68]
data3$Year[69] = "Interest Compounded Annually"
for(i in 4:104){
  for(j in 1:68){
    prod = 1
    y = j+29
