...
Inflate_Bill_x = Inflate_Bill
Inflate_Bill_x[,2:3] = 1 + Inflate_Bill_x[,2:3]
Inflate_Bill_Monthly = Inflate_Bill
Inflate_Bill_Monthly = Inflate_Bill_Monthly[1:1153,]
Inflate_Bill_Monthly[,2:3]=NA

for(j in 2:3){
  for(i in 1:1153){
    x=i
    y=x+11
    Inflate_Bill_Monthly[i,j] = prod(Inflate_Bill_x[x:y, j])-1}}

Inflate_Bill_Monthly_1a = Inflate_Bill_Monthly
Inflate_Bill_Monthly_1g = Inflate_Bill_Monthly
Inflate_Bill_Monthly_1g[,2:3] = 1 + Inflate_Bill_Monthly_1g[,2:3]
Inflate_Bill_Monthly_a = Inflate_Bill_Monthly_1a
Inflate_Bill_Monthly_a[,2:3] = NA
Inflate_Bill_Monthly_g = Inflate_Bill_Monthly_1g
Inflate_Bill_Monthly_g[,2:3] = NA

for(j in 2:3){
  for(i in 1:794){
    x=i
    y=x+359
    Inflate_Bill_Monthly_g[i,j] = prod(Inflate_Bill_Monthly_1g[x:y, j])^(1/360)-1
    Inflate_Bill_Monthly_a[i,j] = sum(Inflate_Bill_Monthly_1a[x:y, j])/360}}

Inflate_Bill_Monthly_g["diff"] = Inflate_Bill_Monthly_g$`Ibbotson® SBBI® US (30-Day) Treasury Bills`-Inflate_Bill_Monthly_g$`Ibbotson® SBBI® US Inflation`
Inflate_Bill_Monthly_a["diff"] = Inflate_Bill_Monthly_a$`Ibbotson® SBBI® US (30-Day) Treasury Bills`-Inflate_Bill_Monthly_a$`Ibbotson® SBBI® US Inflation`

Annual_Inflate_Bill = data.frame(matrix(ncol=4, nrow=97))

x=1
for(i in 1:1153){
  if((i-1)%%12 ==0){
    Annual_Inflate_Bill[x,] = Inflate_Bill_Monthly[i,]
    x = x + 1}}
Annual_Inflate_Bill$Date = Data$Date
...
annual_port = annual[, c(1, seq(6,306,3))]
annual_d = data.frame(matrix(ncol=102,nrow=96))
colnames(annual_d) = colnames(annual_port)

for(i in 2:102){
  for(j in 2:97){
    annual_d[j-1, i] = annual_port[j,i] - annual_port[j-1,i]}}
annual_d[,1] = Data$Date[2:97]


annual_dd=annual_d
for(i in 2:102){
  for(j in 2:96){
    if(annual_d[j-1,i] < 0 & annual_d[j,i] < 0){
      annual_dd[j,i] = annual_dd[j,i] + annual_dd[j-1,i]
    }}}
    
    annual_dd[j, i] = max(annual_d[x:y, i])}}

colnames(annual_dd) = colnames(annual_d)
annual_dd = data.frame(matrix(ncol=102,nrow=67))
colnames(annual_dd) = colnames(annual_d)
for(i in 2:102){
  for(j in 1:67){
    x=j
...
annual_draw = data.frame(matrix(ncol=5,nrow=101))
colnames(annual_draw) = c("Allocation", "min", "max", "mean", "std")
annual_draw[,1] = colnames(annual_d)[2:102]
for(i in 1:101){
  annual_draw[i, 2] = min(annual_dd[,i+1])
  annual_draw[i, 3] = max(annual_dd[,i+1])
  annual_draw[i, 4] = sum(annual_dd[,i+1])/67
  annual_draw[i, 5] = sqrt(var(annual_dd[,i+1]))}
...
signif_pts0["Rank"] = 5-rank(signif_pts0$Arith_RRR)
signif_pts0["Rank_Color"] = NA
vect_col = vector()
for(i in 1:4){
  if(as.integer(signif_pts0$Rank[i]) == 1){
...
#     Arith_Sharpe_3_without[j, i+2] = Arith_3[j, i+2] - Inflate_Bill_Monthly_a$diff[j]
#     Arith_Sharpe_3_with[j, i+2] = Arith_Sharpe_3_without[j, i+2]/STD_3[j, i+2]
#     Geom_rrr_3[j, i+2] = STD_3[j, i+2]/Geom_3[j, i+2]
#     Geom_Sharpe_3_without[j, i+2] = Geom_3[j, i+2] - Inflate_Bill_Monthly_g$diff[j]
...
for(i in 0:100){
  for(j in 1:794){
    ...
    Arith_Sharpe_3a_without[j, i+2] = Arith_3a[j, i+2] - Inflate_Bill_Monthly_a$diff[j]
    Arith_Sharpe_3a_with[j, i+2] = Arith_Sharpe_3a_without[j, i+2]/STD_3a[j, i+2]
    Geom_rrr_3a[j, i+2] = STD_3a[j, i+2]/Geom_3a[j, i+2]
    Geom_Sharpe_3a_without[j, i+2] = Geom_3a[j, i+2] - Inflate_Bill_Monthly_g$diff[j]
