...
histout=apply(data,2,hist, i=i+1, main = paste0("Alpha = ", i-1))
...
```{r}
ggplot2::ggplot(data = df[1067:1163,], mapping = aes(x=X1), freq=FALSE) + 
  geom_histogram(aes(y = ..density..))+
  stat_function(fun = dnorm, colour = "red",
                      arg = list(mean = mean(df$X1, na.rm = TRUE),
                                 sd = sd(df$X1, na.rm = TRUE)))+
  scale_x_continuous(name = "Total Returns",
                     breaks = seq(-0.15, 0.35, 0.1),
                     limits=c(-0.15, 0.35)) +
